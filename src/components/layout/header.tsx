import { Bell, Menu, UserRound, Volume2, VolumeX } from 'lucide-react';
...
export function Header({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-header items-center gap-3 border-b border-border/40 bg-white/95 px-4 sm:px-6">
      <IconButton
        label="فتح القائمة"
        className="lg:hidden"
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 hidden truncate text-xs text-muted-foreground/80 sm:block">
          {current?.description ?? 'الإدارة المركزية للمنصة'}
        </p>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        ...
        {canReadNotifications && (
          ...
          >
            ...
            <span
              className="absolute end-2 top-2 size-1.5 rounded-full bg-critical ring-2 ring-white"
              aria-hidden="true"
            />
          </Link>
        )}
