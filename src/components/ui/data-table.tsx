...
export function DataTable<TData>({ data, columns, getRowId, loading = false,
  ...
  // Values passed from the parent work as controlled state.
  ...
  const changeSorting = useCallback(
    (
      updater:
        | SortingState
        | ((current: SortingState) => SortingState),
    ) => {
      const nextSorting =
      ...
      if (
        sameSorting(nextSorting, sorting) &&
        pagination.pageIndex === 0
      ) {
        return;
      }
  ...
  const selectedRows = useMemo(() => {
    ...
    return safeData.filter((row, index) =>
      selectedIds.has(stableGetRowId(row, index)),
    );
  }, [
    safeData,
    rowSelection,
  ...
  // Remove old selected rows when server data changes.
  useEffect(() => {
    if (
      !manualPagination &&
      !manualFiltering
  ...
  const externalPageCount = Number.isFinite(pageCount)
    ? Math.max(Math.trunc(pageCount ?? 1), 1)
    : 1;
  ...
  const countLabel =
    selectedRows.length > 0
      ? `تم تحديد ${formatArabicCount(selectedRows.length)} ${
          selectedRows.length === 1 ? 'عنصر' : 'عناصر'
        }`
      : totalCount !== undefined
        ? `إجمالي العناصر: ${formatArabicCount(totalCount)}`
        : null;
  ...
  return (
    ...
    >
      {(enableSearch || countLabel) && (
        <div className="flex min-h-9 flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
          {enableSearch ? (
          ...
          {countLabel && (
            <div className="shrink-0 text-[12px] font-normal text-muted-foreground">
              {countLabel}
            </div>
          )}
        </div>
      ...
      {selectedRows.length > 0 && selectionActions && (
        <div className="flex flex-col gap-2.5 rounded-xl border border-primary/10 bg-primary/[0.035] px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between">
          <span className="text-[12px] font-medium text-foreground">
            تم تحديد{' '}
            {formatArabicCount(selectedRows.length)}{' '}
            من عناصر الصفحة الحالية
          </span>
                        ...
                        : header.column.getCanSort()
                          ? (
                              <Button
                                variant="ghost"
                                size="sm"
                                className="-mx-2 h-8 gap-1.5 rounded-lg px-2 text-[12px] font-medium text-muted-foreground hover:bg-primary/[0.04] hover:text-foreground"
              ...
              {loading
                ...
                : visibleRows.map((row) => (
                    <tr
                      ...
                      onClick={(event) =>
                        activateRow(row.original, event)
                      }
                      onKeyDown={(event) =>
                        activateRow(row.original, event)
                      }
                      className={cn(
                        'group transition-colors duration-150 hover:bg-primary/[0.025]',
                        onRowClick &&
                          'cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary/20',
                        row.getIsSelected() &&
                          'bg-primary/[0.045] hover:bg-primary/[0.06]',
                      )}
                    >
      ...
      <div className="flex flex-col gap-3 px-0.5 md:flex-row md:items-center md:justify-between">
        <label className="flex items-center gap-2 text-[12px] font-normal text-muted-foreground">
          <span>عدد الصفوف</span>

          <select
            dir="rtl"
            className="h-9 min-w-[70px] rounded-xl border border-border/50 bg-white px-2.5 text-[12px] font-medium text-foreground shadow-none outline-none transition-colors hover:border-primary/20 hover:bg-primary/[0.02] focus-visible:ring-2 focus-visible:ring-primary/20"
          ...
          >
            {safePageSizeOptions.map((size) => (
              <option
                ...
                value={size}
              >
                {size.toLocaleString('ar-SA-u-nu-latn')}
              </option>
            ))}
          </select>
        </label>
