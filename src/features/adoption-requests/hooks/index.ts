...
export const useAdoptionRequests = (filters: AdoptionRequestFilters) => useQuery({ queryKey: adoptionKeys.list(filters), queryFn: () => getAdoptionRequests(filters), placeholderData: keepPreviousData });
export const useAdoptionRequest = (id: string) => useQuery({ queryKey: adoptionKeys.detail(id), queryFn: () => getAdoptionRequestById(id), enabled: Boolean(id) });
export const useAdoptionRequestSummary = () => useQuery({ queryKey: adoptionKeys.summary(), queryFn: getAdoptionRequestSummary });
...
function useInvalidate(id?: string) {
  ...
  return async () => {
    await Promise.all([
      ...
      ...(id ? [client.invalidateQueries({ queryKey: adoptionKeys.detail(id) })] : []),
    ]);
  };
}
...
export function useApproveAdoptionRequest(id: string) {
  ...
  return useMutation({ mutationFn: (note?: string) => approveAdoptionRequest(id, note, actor), onSuccess: invalidate });
}

export function useRejectAdoptionRequest(id: string) {
  const actor = useActor();
  const invalidate = useInvalidate(id);
  return useMutation({ mutationFn: (input: RejectAdoptionInput) => rejectAdoptionRequest(id, input, actor), onSuccess: invalidate });
}

export function useDeleteAdoptionRequest(id: string) {
  const actor = useActor();
  const invalidate = useInvalidate();
  return useMutation({ mutationFn: (reason: string) => deleteAdoptionRequest(id, reason, actor), onSuccess: invalidate });
}

export function useAddAdoptionNote(id: string) {
  const actor = useActor();
  const invalidate = useInvalidate(id);
  return useMutation({ mutationFn: (note: string) => addAdoptionNote(id, note, actor), onSuccess: invalidate });
}
