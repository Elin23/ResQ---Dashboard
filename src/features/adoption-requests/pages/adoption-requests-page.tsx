...
function toParams(filters: AdoptionRequestFilters) {
  ...
  const entries: Array<[string, string | number | undefined]> = [
    ...
    ['publisherType', filters.publisherType],
...
export function AdoptionRequestsPage() {
  ...
  // Sync table pagination and sorting with the URL filters.
  const onState = useCallback(
    (state: DataTableQueryState) => {
      const sort = state.sorting[0];

      const next =
        sort && ['submittedAt', 'updatedAt', 'status'].includes(sort.id)
          ? (sort.id as AdoptionRequestFilters['sortBy'])
          : undefined;

      const sortDirection = next
        ? sort?.desc
          ? 'desc'
          : 'asc'
        : undefined;
      ...
      if (
        ...
        next !== filters.sortBy ||
        (next && sortDirection) !== filters.sortDirection
      ) {
        update({
    ...
    <EmptyState
      title={
        filters.status === 'PENDING_REVIEW'
          ? 'لا توجد عروض جديدة بانتظار المراجعة.'
          : active
            ? 'لا توجد عروض تطابق الفلاتر الحالية.'
            : 'لا توجد عروض تبني حتى الآن.'
      }
      description={
        active
          ? 'جرّب تعديل الفلاتر أو مسحها.'
          : 'ستظهر هنا طلبات نشر الحيوانات للتبني التي يرسلها المستخدمون والجمعيات.'
      }
      action={
        active ? (
          <Button
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="عروض التبني"
        description="مراجعة طلبات نشر الحيوانات للتبني، ثم متابعة طلبات المتبنين ورد الناشر بعد النشر."
        breadcrumbs={[
        ...
        actions={
          <ExportMenuButton
            ...
            columns={[
              {
                label: 'الرقم',
                value: (item: AdoptionRequest) => item.id,
              },
              {
                label: 'الحيوان',
                value: (item: AdoptionRequest) =>
                  item.animal.name ??
                  adoptionAnimalSpeciesLabels[item.animal.species],
              },
              {
                label: 'النوع',
                value: (item: AdoptionRequest) =>
              ...
              {
                label: 'الناشر',
                value: (item: AdoptionRequest) => item.publisher.name,
              },
              {
                label: 'نوع الناشر',
                value: (item: AdoptionRequest) =>
                  adoptionPublisherTypeLabels[item.publisher.type],
              },
              {
                label: 'الحالة',
                value: (item: AdoptionRequest) =>
                  adoptionStatusLabels[item.status],
              },
              {
                label: 'طلبات التبني',
                value: (item: AdoptionRequest) => item.applicationsCount,
              },
              {
                label: 'الموقع',
                value: (item: AdoptionRequest) => item.location,
              },
              {
                label: 'تاريخ الطلب',
                value: (item: AdoptionRequest) =>
