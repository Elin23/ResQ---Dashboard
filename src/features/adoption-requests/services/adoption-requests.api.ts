...
function normalizeDetails(payload: unknown): AdoptionRequestDetails | null { const raw = unwrap(payload); if (raw == null) return null; const b = rec(raw); const applications=arr(b.applications ?? b.adoptionApplications).map(application); const request=normalizeRequest(b.request ?? b.adoptionRequest ?? b); request.applicationsCount=applications.length; request.pendingApplicationsCount=applications.filter(x=>x.status==='PENDING').length; request.acceptedApplicationId=applications.find(x=>x.status==='ACCEPTED')?.id; return { request, applications, timeline: arr(b.timeline ?? b.activity ?? b.events).map(timeline), notes: arr(b.notes ?? b.internalNotes).map(note) }; }
