...
export const operationalMapKeys = {
  all: ['map-directory'] as const,
  data: () => ['map-directory', 'data'] as const,
};
...
export function useCreateMapListing() {
  ...
  return useMutation({
    mutationFn: createMapListing,
    onSuccess: refresh,
  });
}

export function useApproveMapListing() {
  const refresh = useRefresh();

  return useMutation({
    mutationFn: approveMapListing,
    onSuccess: refresh,
  });
}

export function useRejectMapListing() {
  const refresh = useRefresh();

  return useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) =>
      rejectMapListing(id, reason),
    onSuccess: refresh,
  });
}

export function useToggleMapListing() {
  const refresh = useRefresh();

  return useMutation({
    mutationFn: toggleMapListing,
    onSuccess: refresh,
  });
}
