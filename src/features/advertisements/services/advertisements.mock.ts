...
const hours = (offset: number) => new Date(now + offset * 3_600_000).toISOString();
...
for (const ad of rows) {
  const events: AdvertisementTimelineEvent[] = [{ id: `${ad.id}-created`, title: 'تمت إضافة الإعلان من لوحة الإدارة', timestamp: ad.createdAt, tone: 'info' }];
  if (ad.activatedAt) events.unshift({ id: `${ad.id}-active`, title: 'تم نشر الإعلان في التطبيق', timestamp: ad.activatedAt, tone: 'success' });
  if (ad.pausedAt) events.unshift({ id: `${ad.id}-paused`, title: 'تم إيقاف الإعلان مؤقتًا', timestamp: ad.pausedAt, details: ad.pauseReason, tone: 'pending' });
  timelines.set(ad.id, events);
}

function find(id: string) {
  const advertisement = rows.find((item) => item.id === id);
  if (!advertisement) throw new Error('ADVERTISEMENT_NOT_FOUND');
...
function filtered(filters: AdvertisementFilters) {
  ...
  return rows.filter((ad) => {
    ...
    if (filters.dateTo && new Date(ad.createdAt) > new Date(`${filters.dateTo}T23:59:59`)) return false;
    return true;
  });
}
...
export async function getAdvertisements(filters: AdvertisementFilters): Promise<AdvertisementListResult> {
  await mockDelay(80);
  const items = filtered(filters).sort((a, b) => (new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()) * (filters.sortDirection === 'asc' ? -1 : 1));
...
export async function getAdvertisementById(id: string): Promise<AdvertisementDetails | undefined> {
  await mockDelay(60);
  const advertisement = rows.find((item) => item.id === id && item.status !== 'DELETED');
  if (!advertisement) return undefined;
  return { advertisement: clone(advertisement), timeline: clone(timelines.get(id) ?? []) };
}
...
export async function createAdvertisement(input: CreateAdvertisementInput, actor: AdminSession): Promise<Advertisement> {
  ...
  const advertisement: Advertisement = {
    id,
    advertiser: { type: 'CLIENT', name: input.ownerName },
    ...
    createdAt: timestamp,
    updatedAt: timestamp,
    performance: { mockData: true },
  };
  rows.unshift(advertisement);
  timelines.set(id, [{ id: `${id}-created`, title: 'تمت إضافة الإعلان من لوحة الإدارة', actor: actor.name, timestamp, tone: 'info' }]);
...
export async function activateAdvertisement(id: string, actor: AdminSession): Promise<Advertisement> {
  await mockDelay(70);
  const ad = find(id);
  if (!ad.paid) throw new Error('PAYMENT_NOT_CONFIRMED');
  const timestamp = new Date().toISOString();
  ad.status = 'ACTIVE'; ad.activatedAt = timestamp; ad.pausedAt = undefined; ad.pauseReason = undefined; ad.updatedAt = timestamp;
  timelines.set(id, [{ id: `${id}-active-${Date.now()}`, title: 'تم نشر الإعلان في التطبيق', actor: actor.name, timestamp, tone: 'success' }, ...(timelines.get(id) ?? [])]);
  recordAdminAuditEvent(actor, { action: 'ADVERTISEMENT_APPROVED', resource: { type: 'ADVERTISEMENT', id, label: ad.publicationTitle }, newValue: { status: 'ACTIVE' }, metadata: { source: 'إدارة الإعلانات' } });
  return clone(ad);
}
...
export async function pauseAdvertisement(id: string, reason: string, actor: AdminSession): Promise<Advertisement> {
  await mockDelay(70);
  const ad = find(id);
  if (ad.status !== 'ACTIVE') throw new Error('INVALID_STATUS');
  const timestamp = new Date().toISOString();
  ad.status = 'PAUSED'; ad.pausedAt = timestamp; ad.pauseReason = reason; ad.updatedAt = timestamp;
  timelines.set(id, [{ id: `${id}-paused-${Date.now()}`, title: 'تم إيقاف الإعلان مؤقتًا', actor: actor.name, timestamp, details: reason, tone: 'pending' }, ...(timelines.get(id) ?? [])]);
  recordAdminAuditEvent(actor, { action: 'ADVERTISEMENT_PAUSED', resource: { type: 'ADVERTISEMENT', id, label: ad.publicationTitle }, reason, newValue: { status: 'PAUSED' }, metadata: { source: 'إدارة الإعلانات' } });
  return clone(ad);
}
...
export async function deleteAdvertisement(id: string, reason: string, actor: AdminSession): Promise<void> {
  await mockDelay(65);
  const ad = find(id);
  ad.status = 'DELETED'; ad.updatedAt = new Date().toISOString();
  recordAdminAuditEvent(actor, { action: 'ADVERTISEMENT_EXPIRED', resource: { type: 'ADVERTISEMENT', id, label: ad.publicationTitle }, reason, newValue: { status: 'DELETED' }, metadata: { source: 'إدارة الإعلانات' } });
}
...
export async function getAdvertiserAdvertisementSummary(_advertiserType: string, advertiserId?: string): Promise<AdvertisementAdvertiserSummary> {
  await mockDelay(35);
  const scope = advertiserId ? rows.filter((item) => item.advertiser.id === advertiserId && item.status !== 'DELETED') : [];
  return { active: scope.filter((item) => item.status === 'ACTIVE').length, pending: scope.filter((item) => ['DRAFT', 'PENDING_REVIEW', 'SCHEDULED'].includes(item.status)).length, paused: scope.filter((item) => item.status === 'PAUSED').length, recent: clone(scope.slice(0, 3)) };
}
