...
export function AdvertisementCreateDialog({
  ...
  const set = <K extends keyof AdvertisementFormState>(
    key: K,
  ...
  const onImagesSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Limit every advertisement to five images.
      const available = Math.max(0, 5 - images.length);
      const next = await Promise.all(
        files.slice(0, available).map(readImage),
      );

      setImages((current) => [...current, ...next].slice(0, 5));

      if (files.length > available) {
        toast.info('يمكن إضافة خمس صور كحد أقصى لكل إعلان');
      }
    } catch {
  ...
  const transferReferenceValid =
    ...
    form.transferReference.trim().length >= 3;
  ...
  const submit = () => {
    ...
    mutation.mutate(
      ...
      },
      {
        ...
        onError: () => {
          toast.error('تعذر إضافة الإعلان');
        },
      },
    );
  };
          ...
          <label className="text-[12px] font-medium">
            ...
            <div className="mt-1.5">
              <Select
                ...
                options={advertisementPaymentMethods.map((value) => ({
                  value,
                  label: advertisementPaymentMethodLabels[value],
                }))}
              />
            </div>
          </label>
          ...
          {form.paymentMethod === 'TRANSFER' && (
            <label className="text-[12px] font-medium sm:col-span-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  set('transferReference', event.target.value)
                }
                placeholder={
                  form.paid
                    ? 'أدخل رقم الحوالة'
                    : 'يُسجل عند تأكيد التسديد'
                }
              />
            </label>
          )}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-[12px] font-medium sm:col-span-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  set('publicationTitle', event.target.value)
                }
              />
            </label>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-muted-foreground">
                    أضف من صورة واحدة وحتى خمس صور من جهازك.
                  </p>
                </div>
            ...
            <label className="text-[12px] font-medium">
              ...
              <Input
                dir="ltr"
                className="mt-1.5 h-9 rounded-xl text-left"
                value={form.publicationPhone}
                onChange={(event) =>
                  set('publicationPhone', event.target.value)
                }
              />
            </label>
            ...
            <label className="text-[12px] font-medium">
              ...
              <Input
                dir="ltr"
                className="mt-1.5 h-9 rounded-xl text-left"
                value={form.publicationEmail}
                onChange={(event) =>
                  set('publicationEmail', event.target.value)
                }
              />
            </label>
            ...
            <label className="text-[12px] font-medium">
              ...
              <div className="mt-1.5">
                <Select
                  ...
                  options={advertisementPlacements.map((value) => ({
                    value,
                    label: advertisementPlacementConfig[value].label,
                  }))}
                />
              </div>
            </label>
