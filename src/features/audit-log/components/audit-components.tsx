...
export function AuditFilterBar({ filters, options, onChange, onClear }: { filters: AuditFilters; options?: AuditFilterOptions; onChange: (patch: Partial<AuditFilters>) => void; onClear: () => void }) {
  ...
  return (
    <div
      dir="rtl"
      className="flex flex-wrap items-end justify-start gap-2 rounded-xl border border-border/45 bg-white p-3"
    >
      <label className="min-w-[260px] flex-1">
        ...
        <DebouncedSearchInput
          ...
          placeholder="ابحث بالإجراء، المسؤول أو المورد…"
        />
      </label>
      ...
      <label className="min-w-[160px]">
        ...
        <Select
          ...
          options={[
            { value: 'ALL', label: 'كل الموارد' },
            ...(options?.resourceTypes ?? []).map((value) => ({
              value,
              label: auditResourceLabels[value],
            })),
          ]}
        />
      </label>
      ...
      <label className="flex h-9 items-center gap-2 rounded-xl border border-border/45 px-3 text-[12px] font-normal text-muted-foreground">
        <Checkbox
          checked={filters.sensitive ?? false}
          onCheckedChange={(value) =>
            onChange({
              sensitive: value === true ? true : undefined,
              page: 1,
            })
          }
        />
...
function ActorInline({ event }: { event: AuditEvent }) {
  return (
    <div className="flex min-w-0 items-center gap-2">
      ...
      <span className="shrink-0 text-[11px] text-muted-foreground">
        {event.actor.role ?? (event.actor.type === 'SYSTEM' ? 'النظام' : 'إداري')}
      </span>
    </div>
  );
}
...
export function AuditTable({ items, total, pageCount, filters, loading, onStateChange, onOpen }: { items: AuditEvent[]; total: number; pageCount: number; filters: AuditFilters; loading: boolean; onStateChange: (state: DataTableQueryState) => void; onOpen: (event: AuditEvent) => void }) {
  // Keep table columns stable between renders.
  const columns = useMemo<Array<ColumnDef<AuditEvent, unknown>>>(
    () => [
        ...
        cell: ({ row }) => (
          <div className="whitespace-nowrap">
            <span className="text-[12px] text-foreground">
              {formatAuditTimestamp(row.original.timestamp)}
            </span>

            <span className="ms-2 text-[11px] text-muted-foreground">
              {formatAuditRelative(row.original.timestamp)}
            </span>
          </div>
        ),
      },
      {
        ...
        enableSorting: false,
        cell: ({ row }) => (
          <ActorInline event={row.original} />
        ),
      },
      {
        id: 'action',
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <span className="text-[12px] text-foreground">
              {auditActionLabels[row.original.action]}
            </span>

            {sensitiveAuditActions.has(row.original.action) && (
              <Badge tone="critical">
                حساس
              </Badge>
            )}
        ...
        cell: ({ row }) => (
          <div className="flex min-w-0 items-center gap-2">
            <span className="text-[12px] font-medium">
              {auditResourceLabels[row.original.resource.type]}
            </span>

            <span className="max-w-[180px] truncate text-[11px] text-muted-foreground">
              {row.original.resource.label ?? row.original.resource.id}
            </span>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="block max-w-[240px] truncate text-[11px] text-muted-foreground">
            {row.original.reason ?? row.original.metadata?.source ?? '—'}
          </span>
        ),
      },
    ],
  ...
  return (
    <DataTable
      ...
      onRowClick={onOpen}
      rowAriaLabel={(item) => `عرض تفاصيل حدث ${item.id}`}
      rowActions={(item) => (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <IconButton label={`إجراءات ${item.id}`}>
      ...
      emptyState={
        <EmptyState
          title={
            ...
            filters.actorId ||
            filters.sensitive
              ? 'لا توجد أحداث تطابق عوامل التصفية الحالية.'
              : 'لا توجد أحداث مسجلة ضمن الفترة المحددة.'
          }
        />
      }
    />
...
function DetailRow({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="grid gap-1 border-b border-border/35 py-2.5 last:border-0 sm:grid-cols-[8rem_1fr]">
      <dt className="text-[11px] font-normal text-muted-foreground">
        {label}
      </dt>
...
function AuditDiffViewer({ event, canSensitive }: { event: AuditEvent; canSensitive: boolean }) {
  const before = valueRows(event.previousValue, canSensitive);
  const after = valueRows(event.newValue, canSensitive);

  if (!before.length && !after.length) {
    return (
      <p className="text-[12px] text-muted-foreground">
        لا توجد قيم قبل/بعد لهذا الحدث.
      </p>
    );
  }

  // Merge both sides so every changed field appears once.
  const keys = [
    ...new Set([
      ...before.map((item) => item.field),
      ...after.map((item) => item.field),
    ]),
  ];
  ...
  return (
    <div className="space-y-2">
      {keys.map((key) => {
        const previous =
          before.find((item) => item.field === key)?.value ?? '—';

        const next =
          after.find((item) => item.field === key)?.value ?? '—';
        ...
        return (
          ...
          >
            ...
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              ...
              <div className="rounded-lg bg-primary/[0.035] p-2.5">
                <span className="block text-[11px] text-muted-foreground">
                  بعد
                </span>
...
export function AuditDetailModal({ event, onClose }: { event: AuditEvent | null; onClose: () => void }) {
  ...
  // Only show the resource link when the current role can access it.
  const canOpen = !required || permissions.has(required);
  const path = canOpen
    ? resourcePath(event.resource.type, event.resource.id)
    : undefined;

  const canTechnical = permissions.has('audit.technical.read');

  const canSensitive =
    event.resource.type === 'USER'
      ? permissions.has('users.sensitive.read')
      : false;
  ...
  return (
    <Modal
      ...
      title="تفاصيل الحدث"
      description="سجل للقراءة فقط."
    >
      <div className="max-h-[70vh] space-y-4 overflow-y-auto pe-1">
        <dl>
          <DetailRow label="المسؤول">
            <ActorInline event={event} />
          </DetailRow>

          <DetailRow label="الإجراء">
            {auditActionLabels[event.action]}
          </DetailRow>

          <DetailRow label="المورد">
            <div className="flex flex-wrap items-center gap-2">
              <span>
                {auditResourceLabels[event.resource.type]}
              </span>
          ...
          <DetailRow label="السبب">
            {event.reason ?? 'لا يوجد سبب مسجل'}
          </DetailRow>

          <DetailRow label="المصدر">
        ...
        {canTechnical && event.requestContext && (
          <section>
            <h3 className="mb-2 text-[14px] font-semibold">
              بيانات تقنية
            </h3>
            ...
            <dl>
              <DetailRow label="عنوان IP">
                {event.requestContext.ipAddress ?? '—'}
              </DetailRow>

              <DetailRow label="معرف التتبع">
                <span
                  dir="ltr"
                  className="text-[11px]"
                >
...
export function AuditExportButton({ onExport, disabled }: { onExport: () => void; disabled?: boolean }) {
  return (
    <Button
      className="h-9 rounded-xl text-[12px]"
      variant="secondary"
