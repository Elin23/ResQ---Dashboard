...
function operationStatus(report: Report): ReportOperationStatus {
  ...
  return report.assignedOrganization
    ? 'ON_THE_WAY'
    : 'ASSIGNED';
}
...
// Convert an assigned report into the operational shape used by this module.
function toOperation(report: Report): ReportOperationRecord | null {
  if (!report.assignedOrganization) {
    return null;
  }
  ...
  const completedAt =
    report.receivedAt ??
    report.closedAt ??
    (completed ? report.updatedAt : undefined);
  ...
  return {
    ...
    animal: {
      ...
      description: report.animalDescription,
    },
    location: {
...
export async function getReportOperations(filters: CompatFilters = {}) {
  ...
  const rows = result.items
    .map(toOperation)
    .filter(
      (value): value is ReportOperationRecord =>
        value !== null,
    );
...
export async function getReportOperationDashboardSnapshot() {
  const result = await getReportOperations({
    page: 1,
    pageSize: 100,
  });
  ...
  // Treat active operations older than 45 minutes as overdue.
  const overdueCount = active.filter(
    (operation) =>
      Date.now() -
        new Date(operation.assignedAt).getTime() >
      45 * 60_000,
  ).length;
