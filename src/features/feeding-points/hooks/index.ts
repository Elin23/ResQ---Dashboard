...
import { addFeedingPointNote, approveFeedingPoint, deactivateFeedingPoint, getFeedingPointById, getFeedingPoints, getFeedingPointSummary, reactivateFeedingPoint, rejectFeedingPoint, rejectIssue, resolveIssue, reviewFeedingPointRefill, startIssueReview } from '../services/feeding-points.service';
...
export const useFeedingPoints=(filters:FeedingPointFilters)=>useQuery({queryKey:feedingPointKeys.list(filters),queryFn:({signal})=>getFeedingPoints(filters,signal),placeholderData:keepPreviousData});
export const useFeedingPointSummary=()=>useQuery({queryKey:feedingPointKeys.summary(),queryFn:({signal})=>getFeedingPointSummary(signal)});
export const useFeedingPoint=(id:string)=>useQuery({queryKey:feedingPointKeys.detail(id),queryFn:({signal})=>getFeedingPointById(id,signal),enabled:Boolean(id)});
...
export function useDeactivateFeedingPoint(id:string){const actor=useActor(),invalidate=useInvalidate(id),client=useQueryClient();return useMutation({mutationFn:(input:DeactivateFeedingPointInput)=>deactivateFeedingPoint(id,input,actor),onSuccess:async(details)=>{if(details)client.setQueryData(feedingPointKeys.detail(id),details);await invalidate();}});}
export function useReactivateFeedingPoint(id:string){const actor=useActor(),invalidate=useInvalidate(id),client=useQueryClient();return useMutation({mutationFn:()=>reactivateFeedingPoint(id,actor),onSuccess:async(details)=>{if(details)client.setQueryData(feedingPointKeys.detail(id),details);await invalidate();}});}
