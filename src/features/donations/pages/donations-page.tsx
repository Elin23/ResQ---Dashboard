...
function read(params: URLSearchParams): DonationCampaignFilters {
  ...
  return {
    search: params.get('q') ?? '',
    status:
      status && donationCampaignStatuses.includes(status as DonationCampaignStatus)
        ? (status as DonationCampaignStatus)
        : undefined,
...
export function DonationsPage() {
  ...
  // Update the URL without losing unrelated search parameters.
  const update = useCallback(
    (patch: Partial<DonationCampaignFilters>) =>
      commitSearchParams(
        params,
        write({
          ...filters,
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className="min-w-52">
            <p className="text-[12px] font-medium">{row.original.title}</p>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">مستخدم</p>
          </div>
        ),
      },
      {
        id: 'beneficiary',
        header: 'الجمعية المستفيدة',
        enableSorting: false,
        cell: ({ row }) => (
          <span className="text-[12px]">
            {row.original.beneficiaryOrganization.name}
          </span>
        ),
      },
      {
        ...
        header: 'حالة النشر',
        cell: ({ row }) => (
          <DonationCampaignStatusBadge status={row.original.status} />
        ),
      },
      {
        accessorKey: 'raisedAmountMinor',
        header: 'الأرشيف',
        cell: ({ row }) => (
          <div>
            <p className="whitespace-nowrap text-[12px] font-medium">
              {formatMoney(
                row.original.raisedAmountMinor,
                row.original.currency,
              )}
            </p>
            <p className="text-[11px] text-muted-foreground">
              {row.original.donorCount} متبرع
            </p>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="whitespace-nowrap text-[11px] text-muted-foreground">
            {formatDonationDate(row.original.submittedAt)}
          </span>
        ),
      },
    ],
  ...
  const onStateChange = useCallback(
    (state: DataTableQueryState) => {
      ...
      update({
        ...
        sortBy:
          (sort?.id as DonationCampaignFilters['sortBy']) ??
          filters.sortBy,
  ...
  return (
    <div className="space-y-4" dir="rtl">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-[12px] text-muted-foreground/75">
            راجع طلبات نشر حملات التبرع قبل ظهورها، ثم تابع الأرشيف العام دون الوصول إلى بيانات الدفع أو معلومات المتبرعين الخاصة.
          </p>
        </div>

        <ExportMenuButton
          title="حملات التبرع"
          fileName="resq-donation-campaigns"
          rows={query.data?.items ?? []}
          disabled={query.isLoading}
          subtitle="تصدير أرشيف الحملات الظاهر حاليًا دون بيانات دفع خاصة."
          columns={[
            {
              label: 'الحملة',
              value: (item: DonationCampaign) => item.title,
            },
            {
              label: 'الناشر',
              value: (item: DonationCampaign) => item.publisher.name,
            },
            {
              label: 'الجمعية المستفيدة',
              value: (item: DonationCampaign) =>
                item.beneficiaryOrganization.name,
            },
            {
              label: 'الحالة',
              value: (item: DonationCampaign) =>
                donationCampaignStatusLabels[item.status],
            },
            {
              label: 'إجمالي التبرعات',
              value: (item: DonationCampaign) =>
                formatMoney(
                  item.raisedAmountMinor,
                  item.currency,
                ),
            },
            {
              label: 'عدد المتبرعين',
              value: (item: DonationCampaign) => item.donorCount,
            },
            {
              label: 'تاريخ الطلب',
              value: (item: DonationCampaign) =>
                formatDonationDate(item.submittedAt),
            },
          ]}
        />
      </div>
      ...
      <Summary
        ...
        published={summary.data?.publishedCampaigns ?? 0}
        donors={summary.data?.donorCount ?? 0}
        amount={
          summary.data?.total
      ...
      <div className="flex flex-col gap-2 rounded-xl border border-border/45 bg-white p-2.5 md:flex-row md:items-center">
        <label className="relative min-w-0 flex-1">
          ...
          <Input
            ...
            placeholder="ابحث باسم الحملة، الناشر أو الجمعية…"
          />
        </label>
          ...
          onValueChange={(value) =>
            update({
              status:
                value === 'ALL'
                  ? undefined
                  : (value as DonationCampaignStatus),
              page: 1,
            })
          }
          options={[
            {
              value: 'ALL',
              label: 'كل حالات النشر',
            },
            ...donationCampaignStatuses
              .filter((status) => status !== 'DELETED')
              .map((value) => ({
                value,
                label: donationCampaignStatusLabels[value],
              })),
          ]}
        />
      ...
      <DataTable
        ...
        onRowClick={(campaign) => navigate(`/donations/${campaign.id}`)}
        rowAriaLabel={(campaign) => `فتح حملة ${campaign.title}`}
        rowActions={(campaign) => (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <IconButton label={`إجراءات ${campaign.title}`}>
            ...
            <DropdownMenuContent>
              <DropdownMenuItem
                onSelect={() => navigate(`/donations/${campaign.id}`)}
              >
        ...
        emptyState={
          <EmptyState
            title={
              active
                ? 'لا توجد حملات تطابق الفلاتر الحالية.'
                : 'لا توجد حملات تبرع في الأرشيف.'
            }
          />
        }
      />
