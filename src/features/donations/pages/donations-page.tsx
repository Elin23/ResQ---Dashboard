...
export function DonationsPage() {
  ...
  return <div className="space-y-4" dir="rtl">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-1.5 text-[11px] text-muted-foreground/70"><span>الرئيسية</span><span>/</span><span>التبرعات</span></div><h1 className="mt-1 text-[19px] font-medium leading-6">أرشيف حملات التبرع</h1><p className="mt-0.5 text-[12px] text-muted-foreground/75">راجع طلبات نشر حملات التبرع قبل ظهورها، ثم تابع الأرشيف العام دون الوصول إلى بيانات الدفع أو معلومات المتبرعين الخاصة.</p></div><ExportMenuButton title="حملات التبرع" fileName="resq-donation-campaigns" rows={query.data?.items ?? []} disabled={query.isLoading} subtitle="تصدير أرشيف الحملات الظاهر حاليًا دون بيانات دفع خاصة." columns={[{label:'الحملة',value:(item:DonationCampaign)=>item.title},{label:'الناشر',value:(item:DonationCampaign)=>item.publisher.name},{label:'الجمعية المستفيدة',value:(item:DonationCampaign)=>item.beneficiaryOrganization.name},{label:'الحالة',value:(item:DonationCampaign)=>donationCampaignStatusLabels[item.status]},{label:'إجمالي التبرعات',value:(item:DonationCampaign)=>formatMoney(item.raisedAmountMinor,item.currency)},{label:'عدد المتبرعين',value:(item:DonationCampaign)=>item.donorCount},{label:'تاريخ الطلب',value:(item:DonationCampaign)=>formatDonationDate(item.submittedAt)}]} /></div>
