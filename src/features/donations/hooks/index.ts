...
function useActor() {
  ...
  // Donation management actions always require an authenticated admin.
  if (!session) {
...
function useInvalidate(id?: string) {
  ...
  // Keep donation data and dashboard metrics in sync after every mutation.
  return async () => {
    await Promise.all([
      client.invalidateQueries({ queryKey: donationKeys.all }),
      client.invalidateQueries({ queryKey: dashboardKeys.all }),
      ...(id
        ? [client.invalidateQueries({ queryKey: donationKeys.detail(id) })]
        : []),
    ]);
  };
}
...
export function useApproveDonationCampaign(id: string) {
  ...
  return useMutation({
    mutationFn: () => approveDonationCampaign(id, actor),
...
export function useRejectDonationCampaign(id: string) {
  ...
  return useMutation({
    mutationFn: (reason: string) => rejectDonationCampaign(id, reason, actor),
    onSuccess: invalidate,
  });
}
...
export function useDeleteDonationCampaign(id: string) {
  ...
  return useMutation({
    mutationFn: (reason: string) => deleteDonationCampaign(id, reason, actor),
    onSuccess: invalidate,
  });
}