...
import { Activity, AlertTriangle, ArrowLeft, Building2, CircleAlert, CircleDollarSign, Clock3, HeartHandshake, ShieldAlert, UserCheck } from 'lucide-react';
...
const toneClasses = {
  ...
  critical: 'bg-critical/[0.08] text-critical',
  info: 'bg-info/[0.08] text-info',
};

const primaryMetricKeys = ['reports', 'waiting', 'adoptions', 'donations'] as const;
...
export function DashboardMetrics({ metrics }: { metrics: DashboardMetric[] }) {
  ...
  // Prefer the main operational metrics, then fill any missing slots.
  const selectedMetrics = primaryMetricKeys
    .map((iconKey) => metrics.find((metric) => metric.iconKey === iconKey))
  ...
  if (selectedMetrics.length < 4) {
    for (const metric of metrics) {
      ...
      const alreadyExists = selectedMetrics.some(
...
function waitingLabel(minutes: number): string {
  if (minutes < 60) {
    return `منذ ${minutes.toLocaleString('ar-SA-u-nu-latn')} دقيقة`;
  }

  const hours = Math.floor(minutes / 60);

  if (hours < 24) {
    return `منذ ${hours.toLocaleString('ar-SA-u-nu-latn')} ساعة`;
  }

  return `منذ ${Math.floor(hours / 24).toLocaleString('ar-SA-u-nu-latn')} يوم`;
}

function DashboardSectionTitle({ title, description, action }: { title: string; description?: string; action?: ReactNode }) {
  return (
...
export function OperationalOverview({
  ...
  // Keep the dashboard compact by showing only the first three urgent items.
  const visibleItems = attentionItems?.slice(0, 3);

  return (
    <Card className="rounded-xl border border-border/45 bg-white p-0 shadow-none">
      <div className="grid lg:grid-cols-12">
        <section className="min-w-0 p-4 lg:col-span-5 lg:border-e lg:border-border/40">
            ...
            {attentionError ? (
              ...
              visibleItems.map((item) => (
                <Link
                  key={item.id}
                  to={item.target}
                  className="flex min-h-[54px] items-center gap-2.5 rounded-lg px-2 py-1.5 transition-colors duration-150 hover:bg-muted/30"
                >
                  <span
                    className={cn(
                      'flex size-8 shrink-0 items-center justify-center rounded-lg',
                      toneClasses[item.severity],
                    )}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 truncate text-[11px] leading-4 text-muted-foreground/70">
                      {waitingLabel(item.waitingMinutes)}
                      {' · '}
                      {item.detail}
                    </p>
                  </div>
        ...
        <section className="min-w-0 p-4 lg:col-span-7">
          <DashboardSectionTitle
...
export function DashboardWorkOverview({ missions, reports }: { missions: ActiveMission[]; reports: CriticalReport[] }) {
  // Only show the most relevant work items on the dashboard.
  ...
  return (
    <Card className="rounded-xl border border-border/45 bg-white p-0 shadow-none">
      <div className="grid lg:grid-cols-2">
        <section className="min-w-0 p-4 lg:border-e lg:border-border/40">
          <DashboardSectionTitle
            ...
            {visibleMissions.length === 0 ? (
              ...
              visibleMissions.map((mission) => (
                <Link
                  ...
                  className="flex min-h-[50px] items-center gap-2.5 rounded-lg px-2 py-1.5 transition-colors hover:bg-muted/30"
                >
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-primary/[0.06] text-primary">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 items-center gap-1.5">
                      ...
                      <Badge tone={priorityMeta[mission.priority].tone}>
                        {priorityMeta[mission.priority].label}
                      </Badge>
                    </div>
                  ...
                  <StatusBadge status={mission.stage} />
                </Link>
              ))
            )}
          </div>
            ...
            {visibleReports.length === 0 ? (
              ...
              visibleReports.map((report) => (
                <Link
                  ...
                  className="flex min-h-[50px] items-center gap-2.5 rounded-lg px-2 py-1.5 transition-colors hover:bg-muted/30"
                >
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-critical/[0.07] text-critical">
                  ...
                  <Badge tone={report.severity === 'CRITICAL' ? 'critical' : 'pending'}>
                    {report.severity === 'CRITICAL' ? 'حرجة' : 'عالية'}
                  </Badge>
                </Link>
              ))
            )}
    ...
    <div className="space-y-3">
      <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }, (_, index) => (
          <Skeleton
            key={index}
            className="h-[94px] rounded-xl"
          />
        ))}
      </div>

      <Skeleton className="h-[245px] rounded-xl" />
