...
function RangeFilter({ value, onChange }: { value: DashboardRange; onChange: (value: DashboardRange) => void }) {
  ...
  // Close the range menu when clicking outside it or pressing Escape.
  useEffect(() => {
    if (!open) {
      return;
    }
  ...
  return (
    <div ref={containerRef} className="relative shrink-0">
      <button
        ...
        className="flex h-9 min-w-[118px] items-center justify-between gap-3 rounded-xl border border-border/50 bg-white px-3 text-[12px] font-medium text-foreground shadow-none transition-colors duration-150 hover:border-primary/20 hover:bg-primary/[0.025] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20"
      >
      ...
      {open && (
        <div
          ...
          className="absolute end-0 top-full z-40 mt-1.5 w-[150px] overflow-hidden rounded-xl border border-border/50 bg-white p-1 shadow-[0_8px_24px_-16px_rgba(0,0,0,0.22)]"
        >
          {rangeOptions.map((option) => {
            ...
            return (
              <button
                ...
                className={`flex h-9 w-full items-center justify-between gap-3 rounded-lg px-2.5 text-start text-[12px] transition-colors duration-150 ${
                  selected
                    ? 'bg-primary/[0.07] font-medium text-primary'
                    : 'font-normal text-muted-foreground hover:bg-primary/[0.05] hover:text-primary'
                }`}
              >
...
export function DashboardPage() {
  ...
  // Format the backend refresh timestamp for the dashboard header.
  const updatedAt = summary.data?.generatedAt
    ? format(new Date(summary.data.generatedAt), 'h:mm a', { locale: arSA })
    : '—';
    ...
    <div className="flex min-h-0 flex-col gap-3.5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h1 className="truncate text-[18px] font-semibold leading-6 tracking-tight text-foreground sm:text-[19px]">
            {greeting(session?.name ?? 'أحمد')}
          </h1>

          <p className="mt-0.5 text-[11px] font-normal text-muted-foreground/70">
            آخر تحديث {updatedAt}
          </p>
        </div>
