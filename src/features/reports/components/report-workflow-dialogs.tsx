...
import { useAdminOverrideReportStatus, useAssignReport, useDeleteReport, useEligibleOrganizations } from '../hooks';
import { assignmentSchema, deleteReportSchema, statusOverrideSchema, type AssignmentFormValues, type DeleteReportFormValues, type StatusOverrideFormValues } from '../schemas';
...
function mutationMessage(error: Error): string {
  if (error.message === 'INVALID_REPORT_STATE') {
    return 'حالة البلاغ الحالية لا تسمح بهذا الإجراء.';
  }

  if (error.message === 'ORGANIZATION_NOT_AVAILABLE') {
    return 'الجمعية المحددة غير متاحة حاليًا.';
  }

  if (error.message === 'REPORT_NOT_FOUND') {
    return 'البلاغ غير موجود أو تم حذفه.';
  }

  return 'تعذر تنفيذ الإجراء. حاول مرة أخرى.';
}

export function ReportWorkflowDialog({ report, workflow, open, onOpenChange, onDeleted }: { report: Report; workflow: ReportWorkflow; open: boolean; onOpenChange: (open: boolean) => void; onDeleted?: () => void }) {
  if (workflow === 'assign') {
    return (
      <AssignmentDialog
        report={report}
...
function AssignmentDialog({ report, open, onOpenChange }: { report: Report; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const {
    ...
    defaultValues: {
      organizationId: report.assignedOrganization?.id ?? '',
    },
  });
            ...
            return (
              <button
                ...
                className={`flex w-full items-center justify-between gap-4 border-b border-border/35 px-3 py-2.5 text-start transition-colors last:border-b-0 ${
                  selected
                    ? 'bg-primary/[0.055] text-foreground'
                    : 'hover:bg-muted/25'
                }`}
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 truncate text-[11px] text-muted-foreground/70">
                    ...
                    {organization.distanceKm
                      ? ` · ${organization.distanceKm} كم تقريبًا`
                      : ''}
                  </p>
                </div>
                ...
                <div className="shrink-0 text-end text-[11px] text-muted-foreground/70">
                  <p>
                    {organization.activeReports} بلاغات نشطة
                  </p>
...
function StatusOverrideDialog({ report, open, onOpenChange }: { report: Report; open: boolean; onOpenChange: (open: boolean) => void }) {
          ...
          <div className="mt-1.5">
            <Select
              ...
              onValueChange={(value) =>
                setValue(
                  'status',
                  value as StatusOverrideFormValues['status'],
                  { shouldValidate: true },
                )
              }
              options={Object.entries(reportStatusLabels).map(
                ([value, label]) => ({
                  value,
                  label,
                }),
...
function DeleteDialog({ report, open, onOpenChange, onDeleted }: { report: Report; open: boolean; onOpenChange: (open: boolean) => void; onDeleted?: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Deletion is reserved for invalid or duplicate reports, not completed cases. */}
