...
import { formatRelativeTime, formatReportDate } from '../utils';
...
function SectionTitle({ title, description }: { title: string; description?: string }) {
  return (
    <div>
      ...
      {description && (
        <p className="mt-0.5 text-[12px] leading-5 text-muted-foreground/70">
          {description}
        </p>
      )}
    </div>
...
function Info({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="min-w-0">
      ...
      <dd className="mt-1 min-w-0 text-[12px] font-medium text-foreground">
        {value}
      </dd>
    </div>
  );
...
export function ReportOverview({ details }: { details: ReportDetails }) {
  ...
  return (
    <div>
      <SectionTitle
        title="ملخص الحالة"
        description="المعلومات الأساسية المرتبطة بالبلاغ المنشور."
      />

      <dl className="mt-4 grid gap-x-8 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
        <Info
          label="نوع الحيوان"
          value={animalTypeLabels[report.animalType]}
        />
        ...
        <Info
          label="الجمعية الحالية"
          value={report.assignedOrganization?.name ?? 'لم تستلمها جمعية بعد'}
        />

        <Info
          label="الموقع"
          value={`${report.governorate}${report.city ? ` — ${report.city}` : ''}`}
        />

        <Info
          label="وصف الحيوان"
          value={report.animalDescription ?? 'غير محدد'}
        />
      </dl>
...
export function ReportMediaSection({ details }: { details: ReportDetails }) {
  return (
    <div>
      <SectionTitle
        title="الوسائط"
        description={`${details.report.media.length} ملفات مرتبطة بالبلاغ`}
      />
...
export function ReportLocationSection({ details }: { details: ReportDetails }) {
  ...
  const title =
    `${report.governorate}${report.city ? ` — ${report.city}` : ''}`;
...
export function ReporterCard({ details }: { details: ReportDetails }) {
  ...
  // Copy the reporter phone only when it exists.
      ...
      <div className="mt-3 space-y-2 text-[12px]">
        ...
        {reporter.email && (
          <div className="flex items-center justify-between gap-3">
            ...
            <span dir="ltr" className="max-w-[170px] truncate font-medium text-foreground">
              {reporter.email}
            </span>
          </div>
        )}
  ...
  return (
    <div>
      <SectionTitle
        ...
        description="داخلية ولا تظهر للمبلّغ أو الجمعية."
      />
      ...
      <PermissionGuard permission="reports:update">
        ...
        >
          <Textarea
            {...register('note')}
            placeholder="أضف ملاحظة تشغيلية…"
          />
          ...
          <Button
            size="sm"
            className="mt-2 h-8 rounded-lg px-2.5 text-[11px]"
            type="submit"
