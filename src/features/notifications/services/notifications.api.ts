...
export async function updateNotificationTemplate(key:string,input:TemplateUpdateInput){const current=await getNotificationTemplate(key);if(!current)throw new Error('تعذر العثور على قالب الإشعار.');await apiClient.patch<unknown>(`/api/dashboard/notifications/templates/${encodeURIComponent(key)}`,{name:current.name,titleTemplate:input.titleTemplate,bodyTemplate:input.bodyTemplate,isActive:input.enabled});return getNotificationTemplate(key);}
export async function getNotificationTargetSources(signal?:AbortSignal){const [sourcePayload,userPayload]=await Promise.all([apiClient.get<unknown>('/api/dashboard/notifications/target-sources',signal),apiClient.get<unknown>('/api/dashboard/users?page=1&pageSize=200',signal)]);const rawSources=unwrap(sourcePayload);const sources=arr(rawSources);const organizations=sources.filter(x=>String(rec(x).type??'').toUpperCase()==='ORGANIZATION').map(x=>{const q=rec(x);return{id:id(q.id),name:str(q.name)??id(q.id)}});const ur=rec(unwrap(userPayload));const users=arr(ur.items??ur.users??unwrap(userPayload)).map(x=>{const q=rec(x);return{id:id(q.id,q.userId),name:str(q.fullName,q.name,q.userName,q.email)??id(q.id,q.userId)}});return{organizations,users};}
