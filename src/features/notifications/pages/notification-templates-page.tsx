  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="رسائل نصية آمنة يستهلكها الخادم عند أحداث المنتج؛ لا تمثل بثًا يدويًا."
        breadcrumbs={[
          { label: 'الرئيسية', href: '/dashboard' },
          { label: 'الإشعارات', href: '/notifications' },
          { label: 'القوالب التلقائية' },
        ]}
      />
      ...
      {query.isLoading ? (
        ...
        // Render the current server-backed template collection after loading.
        <NotificationTemplatesList templates={query.data ?? []} />
      )}
    </div>
  );
}