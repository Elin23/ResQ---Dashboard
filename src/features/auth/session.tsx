...
export interface AdminSession extends AuthenticatedAdmin { avatarUrl?: string; }
...
function safeStorageRemove(storage: Storage, key: string): void {
  try { storage.removeItem(key); } catch { /* storage may be unavailable in hardened browser contexts */ }
}
...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const setDevelopmentRole = useCallback((role: AdminRole) => {
    ...
    clearStoredSession();
    if (!safeStorageSet(window.sessionStorage, STORAGE_KEY, JSON.stringify(next))) return;
    setSession(next);
  }, []);
  ...
  useEffect(() => {
    const onStorage = (event: StorageEvent) => {
      ...
      // Keep permissions and identity synchronized when another tab logs in/out.
  ...
  const value = useMemo<SessionContextValue>(() => ({ session, login, logout, setDevelopmentRole }), [session, login, logout, setDevelopmentRole]);
