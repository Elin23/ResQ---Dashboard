...
export function exportTableToExcel<T>({ title, fileName, columns, rows, subtitle }: ExportTableOptions<T>) {
  ...
  const body = rows.map((row) => `<tr>${columns.map((column) => `<td>${escapeHtml(safeCell(column.value(row)))}</td>`).join('')}</tr>`).join('');
...
export async function exportTableToPdf<T>({ title, fileName, columns, rows, subtitle }: ExportTableOptions<T>) {
  ...
  const tableRows = rows.map((row) => `<tr>${columns.map((column) => `<td style="border-bottom:1px solid #ece8e3;padding:12px 10px;text-align:right;vertical-align:middle;font-size:13px">${escapeHtml(safeCell(column.value(row)))}</td>`).join('')}</tr>`).join('');
  const generatedAt = new Intl.DateTimeFormat('ar-SY-u-nu-latn', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date());
  ...
  host.innerHTML = `
    ...
    <table style="width:100%;border-collapse:collapse;margin-top:26px;direction:rtl">
      <thead><tr>${columns.map((column) => `<th style="background:#faf7f3;border-bottom:1px solid #e6ded6;padding:12px 10px;text-align:right;font-size:13px;font-weight:600">${escapeHtml(column.label)}</th>`).join('')}</tr></thead>
  ...
  try {
    const canvas = await html2canvas(host, { scale: 1.5, backgroundColor: '#ffffff', useCORS: true, logging: false });
