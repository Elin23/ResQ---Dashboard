...
export type SemanticStatus =
  ...
  | 'report:UNDER_REVIEW'
  | 'report:VERIFIED'
  | 'report:WAITING_FOR_ORGANIZATION'
  | 'report:ASSIGNED'
  | 'report:IN_PROGRESS'
  | 'report:RESCUED'
  | 'report:CLOSED'
  ...
  | 'animal:RESERVED'
  | 'animal:ADOPTED'
  | 'animal:NOT_AVAILABLE'
  | 'animal:DECEASED'
  | 'animal-health:CRITICAL'
  | 'animal-health:NEEDS_TREATMENT'
  | 'animal-health:STABLE'
  | 'animal-health:RECOVERING'
  | 'animal-health:HEALTHY'
  | 'adoption:PENDING_REVIEW'
  | 'adoption:PUBLISHED'
  | 'adoption:REJECTED'
  | 'adoption:ADOPTED'
  ...
  | 'notification:SENT'
  | 'notification:PARTIALLY_SENT'
  | 'notification:FAILED'
  | 'notification:CANCELLED';

export const statusCatalog: Record<string, Record<string, StatusMeta>> = {
  report: {
    EN_ROUTE: { label: 'يتم التوجه لمكان البلاغ', tone: 'info' },
    RECEIVED: { label: 'تم الاستلام', tone: 'success' },
    NEW: { label: 'جديد', tone: 'info' },
    UNDER_REVIEW: { label: 'قيد المراجعة', tone: 'pending' },
    VERIFIED: { label: 'تم التحقق', tone: 'success' },
    WAITING_FOR_ORGANIZATION: { label: 'بانتظار جمعية', tone: 'pending' },
    ASSIGNED: { label: 'تم الإسناد', tone: 'info' },
    IN_PROGRESS: { label: 'الإنقاذ جارٍ', tone: 'info' },
    RESCUED: { label: 'تم الإنقاذ', tone: 'success' },
    CLOSED: { label: 'مغلق', tone: 'neutral' },
    REJECTED: { label: 'مرفوض', tone: 'critical' },
  },
  animal: {
    RESCUED: { label: 'تم الإنقاذ', tone: 'info' },
    UNDER_TREATMENT: { label: 'تحت العلاج', tone: 'pending' },
    RECOVERING: { label: 'في التعافي', tone: 'info' },
    AVAILABLE_FOR_ADOPTION: { label: 'جاهز للتبني', tone: 'success' },
    RESERVED: { label: 'محجوز', tone: 'pending' },
    ADOPTED: { label: 'تم تبنيه', tone: 'success' },
    NOT_AVAILABLE: { label: 'غير متاح', tone: 'neutral' },
    DECEASED: { label: 'متوفى', tone: 'neutral' },
  },
  'animal-health': {
    CRITICAL: { label: 'حرجة', tone: 'critical' },
    NEEDS_TREATMENT: { label: 'يحتاج علاجاً', tone: 'pending' },
    STABLE: { label: 'مستقرة', tone: 'info' },
    RECOVERING: { label: 'يتعافى', tone: 'info' },
    HEALTHY: { label: 'سليم', tone: 'success' },
  },
  adoption: {
    PENDING_REVIEW: { label: 'بانتظار مراجعة النشر', tone: 'pending' },
    PUBLISHED: { label: 'منشور', tone: 'success' },
    REJECTED: { label: 'مرفوض النشر', tone: 'critical' },
    ADOPTED: { label: 'تم التبني', tone: 'success' },
  },
  organization: {
    PENDING_VERIFICATION: { label: 'تنتظر التحقق', tone: 'pending' },
    ACTIVE: { label: 'فعالة', tone: 'success' },
    SUSPENDED: { label: 'معلقة', tone: 'critical' },
  ...
  'feeding-point': {
    ...
    INACTIVE: { label: 'معطلة', tone: 'neutral' },
    REJECTED: { label: 'مرفوضة', tone: 'critical' },
  },
  donation: {
    PENDING: { label: 'معلقة', tone: 'pending' },
    COMPLETED: { label: 'مكتملة', tone: 'success' },
    FAILED: { label: 'فشلت', tone: 'critical' },
    REFUNDED: { label: 'مستردة', tone: 'info' },
    CANCELLED: { label: 'ملغاة', tone: 'neutral' },
  },
  entity: {
    PENDING: { label: 'قيد التحقق', tone: 'pending' },
    VERIFIED: { label: 'موثّق', tone: 'success' },
    SUSPENDED: { label: 'موقوف', tone: 'critical' },
  },
  support: {
  ...
  advertisement: {
    ...
    PAUSED: { label: 'متوقف مؤقتًا', tone: 'pending' },
    REJECTED: { label: 'مرفوض', tone: 'critical' },
  ...
  content: {
    ...
    SCHEDULED: { label: 'مجدول', tone: 'pending' },
    PUBLISHED: { label: 'منشور', tone: 'success' },
  ...
  notification: {
    ...
    PARTIALLY_SENT: { label: 'إرسال جزئي', tone: 'pending' },
    FAILED: { label: 'فشل', tone: 'critical' },
    CANCELLED: { label: 'ملغي', tone: 'neutral' },
  },
};

// Flatten semantic keys so UI components can resolve status metadata directly.
...
export function getStatusMeta(status: SemanticStatus | string | null | undefined): StatusMeta {
  ...
  return semanticStatusMap[status as SemanticStatus] ?? {
    label: 'غير معروف',
    tone: 'neutral',
  };
}